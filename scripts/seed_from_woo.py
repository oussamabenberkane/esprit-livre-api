...
WILAYA_CODE_TO_NAME = {
    ...
    "44": "Ain Defla", "45": "Naâma", "46": "Ain Témouchent",
...
def strip_wilaya(code: str | None) -> str | None:
    """Convert Woo wilaya code (e.g. 'DZ-06') to the name Yalidine/ZR expect (e.g. 'Béjaïa')."""
    if not code:
        return None
    code = str(code).strip()
    if code.upper().startswith("DZ-"):
        code = code[3:]
    return WILAYA_CODE_TO_NAME.get(code, code)
